...
class ImplementationModeTests(unittest.TestCase):
    ...
    def _build_empty_pipeline_results(self) -> tuple[AgentResult, AgentResult, AgentResult, AgentResult]:
        repo_context = {
            "repo_id": self.repo_metadata.repo_id,
            "root_path": self.repo_metadata.root_path,
            "files_used": [],
        ...
        return (
            ...
            AgentResult(
                ...
                task_intent="modify",
                repo_context=repo_context,
                metadata={"change_set": ChangeSet(goal="No-op", files=[])},
            ),
            AgentResult(
                agent_name="draft",
                output_text="# Draft Set",
                task_intent="modify",
                repo_context=repo_context,
                metadata={"draft_set": DraftSet(goal="No-op", files=[])},
            ),
        )
    ...
    def test_implementation_mode_stops_early_when_no_changes_are_generated(self) -> None:
        with patch.object(
            root_agent,
            "run_full_draft_pipeline",
            return_value=self._build_empty_pipeline_results(),
        ), patch.object(
            root_agent,
            "resolve_repo",
            return_value=self.repo_metadata,
        ...
        implementation_result = result.metadata["implementation_result"]
        self.assertFalse(mocked_diff_service.called)
        self.assertFalse(mocked_validation_service.called)
        self.assertEqual(implementation_result.final_status, "no_changes")
        self.assertEqual(implementation_result.run_record.status, "no_changes")
        self.assertEqual(
            [step.name for step in implementation_result.run_record.steps],
            ["draft", "validation", "apply"],
        )
        self.assertEqual(implementation_result.run_record.steps[1].status, "skipped")
        self.assertEqual(implementation_result.run_record.steps[2].status, "skipped")
        self.assertEqual(implementation_result.dry_run_apply_result.skip_reason, "no_changes")
        self.assertEqual(implementation_result.root_cause_summary, "No changes generated by agent")
    ...
    def test_implementation_mode_blocks_real_apply_when_validation_path_is_missing(self) -> None:
        ...
        self.assertEqual(implementation_result.dry_run_apply_result.skip_reason, "validation_failed")
        self.assertIn("validation", implementation_result.root_cause_summary.lower())
        self.assertIsNone(implementation_result.real_apply_result)
        self.assertIn("return 'ok'", (self.repo_root / "src" / "app.py").read_text(encoding="utf-8"))
    ...
    def test_implementation_mode_blocks_real_apply_when_validation_fails(self) -> None:
        ...
        self.assertEqual(implementation_result.dry_run_apply_result.skip_reason, "validation_failed")
        self.assertIn("test", implementation_result.root_cause_summary.lower())
        self.assertIsNone(implementation_result.real_apply_result)
        self.assertIn("return 'ok'", (self.repo_root / "src" / "app.py").read_text(encoding="utf-8"))
    ...
    def test_implementation_mode_skips_pr_when_git_is_unavailable(self) -> None:
        ...
        self.assertIsNone(implementation_result.pull_request_result)
        self.assertTrue(
            any("git repository is not available" in warning.lower() for warning in implementation_result.scm_warnings)
        )
