...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the diploma-dev-team local retrieval index for software development team knowledge. "
            "Recommended sources include Python notes, style guides, framework docs, and coding standards."
        )
    )
...
def ensure_dependencies() -> None:
    ...
    if not settings.openai_api_key:
        raise ValueError("OPENAI_API_KEY is required to build embeddings for diploma-dev-team ingestion.")
...
def rebuild_index(delete_existing: bool) -> None:
    ...
    if not chunks:
        manifest = {
            ...
            "message": (
                "No supported documents were found in diploma-dev-team/data. "
                "Add 10-30 markdown, text, or PDF files with Python docs notes, style guides, "
                "coding standards, or framework references and rerun ingest."
            ),
...
if __name__ == "__main__":
    ...
    print(f"Index artifacts are available in {settings.index_dir}")
