...
using TelemartAnimatedGif.Decoding;

namespace TelemartAnimatedGif
{
    /// <summary>
    ...
    public static class ImageBehavior
    {
        ...
        private static void AnimatedSourceChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (ReferenceEquals(oldValue, newValue))
            {
                ...
                {
                    bool isAnimLoaded = GetIsAnimationLoaded(imageControl);
                    if (!isAnimLoaded)
        ...
        private static void VisibilityChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                ImageAnimationController controller = GetAnimationController(img);
                if (controller != null)
                {
                    if ((bool)e.NewValue)
                    {
        ...
        private static ObjectAnimationUsingKeyFrames GetAnimation(Image imageControl, BitmapSource source)
        {
            ...
            if (animation != null)
                return animation;
            GifFile gifMetadata;
            GifBitmapDecoder decoder = GetDecoder(source, imageControl, out gifMetadata) as GifBitmapDecoder;
        ...
        private static BitmapSource ClearArea(BitmapSource frame, FrameMetadata metadata)
        {
            ...
            {
                Rect fullRect = new Rect(0, 0, frame.PixelWidth, frame.PixelHeight);
                Rect clearRect = new Rect(metadata.Left, metadata.Top, metadata.Width, metadata.Height);
        ...
        private static BitmapDecoder GetDecoder(BitmapSource image, Image imageControl, out GifFile gifFile)
        {
            ...
            if (bmp != null)
            {
                ...
                else if (bmp.UriSource != null)
                {
                    ...
                    {
                        Uri baseUri = bmp.BaseUri ?? (imageControl as IUriContext)?.BaseUri;
                        if (baseUri != null)
            ...
            BitmapSource baseFrame)
        {
            ...
            using (DrawingContext context = visual.RenderOpen())
            {
                if (baseFrame != null)
                {
                    Rect fullRect = new Rect(0, 0, fullSize.Width, fullSize.Height);
                    context.DrawImage(baseFrame, fullRect);
                }

                Rect rect = new Rect(metadata.Left, metadata.Top, metadata.Width, metadata.Height);
        ...
        private static FrameMetadata GetFrameMetadata(BitmapFrame frame)
        {
            ...
            FrameDisposalMethod disposalMethod = (FrameDisposalMethod) metadata.GetQueryOrDefault("/grctlext/Disposal", 0);
        ...
        private static FrameMetadata GetFrameMetadata(GifFrame gifMetadata)
        {
            ...
            GifGraphicControlExtension gce = gifMetadata.Extensions.OfType<GifGraphicControlExtension>().FirstOrDefault();
            if (gce != null)
            {
                if (gce.Delay != 0)
