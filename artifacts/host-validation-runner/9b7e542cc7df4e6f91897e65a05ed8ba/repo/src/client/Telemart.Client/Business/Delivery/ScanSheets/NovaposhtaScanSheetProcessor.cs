...
{
    internal sealed class NovaposhtaScanSheetProcessor : IScanSheetProcessor
    {
        public NovaposhtaScanSheetProcessor(IMediator mediator)
        {
            Mediator = mediator;
        }
        ...
        public async Task PrintAsync(CreateScanSheetForEntitiesModel model)
        {
            foreach (Task<Unit> task in model.Result.Select(x => Mediator.Send(new PrintScanSheetRequest(x.Ref, x.Link, true))))
            {
                await task;
        ...
        {
            return new CreateScanSheetRequest(orderIds, ScanSheetPrefix.np);
        }

        public IRestClientGatewayRequest<Result<ScanSheetCreateResponse[]>> CreateEntityRequest(int entityType, int[] entityIds)
        {
            return new CreateScanSheetForEntitiesRequest(entityType, entityIds, ScanSheetPrefix.np);
        }
    }
}