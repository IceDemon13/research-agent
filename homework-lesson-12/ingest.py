"""Build the local retrieval index for the Research Agent's local-context tool."""
...
try:
    ...
    PdfReader = None
    PDF_IMPORT_ERROR: ImportError | None = exc
else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the homework lesson 12 local retrieval index.")
...
def ensure_dependencies() -> None:
    ...
    if not settings.openai_api_key:
        raise ValueError("OPENAI_API_KEY is required to build embeddings for ingestion.")
