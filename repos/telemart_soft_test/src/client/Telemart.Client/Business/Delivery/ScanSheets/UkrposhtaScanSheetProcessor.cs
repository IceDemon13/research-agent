...
{
    internal sealed class UkrposhtaScanSheetProcessor : IScanSheetProcessor
    {
        public UkrposhtaScanSheetProcessor(IMediator mediator)
        {
            Mediator = mediator;
        }
        ...
        {
            foreach (Task<Unit> task in model.Result.Select(x => Mediator.Send(new PrintScanSheetRequest(x.Ref, x.Link, true, x.Base64Pdf))))
            {
                await task;
            }
        }
        ...
        {
            return new CreateScanSheetRequest(orderIds, ScanSheetPrefix.up);
        }
